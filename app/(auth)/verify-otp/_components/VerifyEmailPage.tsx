...
async function verifyOtp(payload: VerifyPayload) {
  ...
  const response = await fetch(`${apiBaseUrl.replace(/\/+$/, '')}/auth/verify`, {
    method: 'POST',
    headers: {
  ...
  const data: VerifyResponse | null = await response.json().catch(() => null)
  const hasExplicitFailure = data?.status === false || data?.success === false

  if (!response.ok || hasExplicitFailure) {
    throw new Error(
      data?.message || data?.error || 'OTP verification failed. Please try again.'
    )
  }
        ...
        <div className="relative z-10 w-full max-w-[560px] bg-[#FFFFFF14]  p-6 md:p-8 border border-[#333333] rounded-[12px]">
          ...
          <div className="space-y-6">
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <Button
                type="submit"
                disabled={!otp.every(v => v) || isExpired || verifyMutation.isPending}
